...
function drawCalendar() {
   ...
   // Write the Calendar with the DOM.
   // Nick D.
   //
	
	//doInnerHTML(idtag + "Div", beginTable + dateControls + calHeader + curHTML + closeControls)
...
function createCalendarWidget() {
	...
	if (document.getElementById) {
		...
		input.setAttribute("class", "DateField");
		input.setAttribute("size", "10");
		input.setAttribute("value", "");
		if (isEditable == "NO_EDIT")
		input.setAttribute("readonly", "readonly");
		if (hasIcon != "ICON")
		input.setAttribute("onclick", "drawCalendar('" + idtag + "')");
		document.getElementById("cal_" + idtag).appendChild(input);
		
		// Create the Image if there needs to be one
		if (hasIcon == "ICON") {
			var image=document.createElementNS("http://www.w3.org/1999/xhtml","img");
			image.setAttribute("src", iconPath);
			image.setAttribute("class", "CalIcon");
			image.setAttribute("id", idtag + "Icon");
			image.setAttribute("alt", "Click to Change Date");
			image.setAttribute("onmousedown", "clickWidgetIcon('" + idtag + "');");
			document.getElementById("cal_" + idtag).appendChild(image);
		}
		
		// Make the Calendar
		var br=document.createElementNS("http://www.w3.org/1999/xhtml","br");
		document.getElementById("cal_" + idtag).appendChild(br);
		
		var calendar=document.createElementNS("http://www.w3.org/1999/xhtml","div");
		calendar.setAttribute("id", idtag + "Div");
		calendar.setAttribute("style", "background: #ffffff; position: absolute; display:none;");
		document.getElementById("cal_" + idtag).appendChild(calendar);
	}
